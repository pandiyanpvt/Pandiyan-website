    ...
    return (
        <Section id="testimonials" title="Client Voices">
            <motion.div
                ...
                className="grid md:grid-cols-3 gap-6 sm:gap-8 md:gap-10 lg:gap-12"
            >
                {testimonials.map((t, idx) => (
                    <motion.div
                        key={idx}
                        variants={itemVariants}
                        className="p-5 sm:p-6 md:p-8 rounded-2xl sm:rounded-3xl bg-white border border-neutral-100 relative group hover:-translate-y-2 transition-all duration-500 shadow-sm hover:shadow-xl"
                    >
                        <div className="text-yellow-400 mb-4 sm:mb-6 flex gap-0.5 sm:gap-1">
                            {[...Array(5)].map((_, i) => (
                                <svg key={i} className="w-4 h-4 sm:w-5 sm:h-5 fill-current shrink-0" viewBox="0 0 20 20">
                                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                                </svg>
                            ))}
                        </div>
                        <p className="text-neutral-600 italic mb-5 sm:mb-8 leading-relaxed text-sm sm:text-base">"{t.content}"</p>
                        <div>
                            <div className="text-neutral-900 font-bold text-sm sm:text-base">{t.name}</div>
                            <div className="text-neutral-500 text-[10px] sm:text-xs uppercase tracking-widest mt-1">{t.role}</div>
                        </div>
                        <div className="absolute top-4 right-4 sm:top-6 sm:right-6 md:top-8 md:right-8 text-neutral-900/5 pointer-events-none">
